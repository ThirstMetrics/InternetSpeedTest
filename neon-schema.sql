-- SpeedTest Database Schema for Neon PostgreSQL
-- Run this in Neon SQL Editor after enabling PostGIS
...
-- 4. Location aggregates table (materialized from speed_tests)
...
CREATE INDEX idx_speed_tests_user ON speed_tests (user_id);
...
BEGIN
  ...
  FROM locations
  WHERE ST_DWithin(location, v_point, 50)
...
CREATE OR REPLACE FUNCTION get_nearby_locations(
  ...
  p_radius_meters DECIMAL DEFAULT 16093,
